from common.misc_util import orthogonal_init, xavier_uniform_init
...
class Flatten(nn.Module):
    def forward(self, x):
        return torch.flatten(x, start_dim=1) # Changed 1 to 0
...
#             Flatten(),
#             nn.Linear(in_features=16*8*8, out_features=512), nn.ReLU() # changed 64*7*7 to 16*8*8
#         )
#         self.output_dim = 512
#         self.apply(orthogonal_init)
...
class ImpalaModel(nn.Module):
    def __init__(self,
        ...
        self.fc = nn.Linear(in_features=32*scale * 8*8 * 4*4, out_features=256) # 32 not 16
